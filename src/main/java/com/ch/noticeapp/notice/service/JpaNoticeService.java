...
public class JpaNoticeService {
    ...
    //EntityManager 쿼리의 즉시 반영등을 처리할 수 있다.
    @PersistenceContext
    ...
    public ResponseNotice regist(RequestNotice requestNotice){
        ...
        notice 를 영속성(Persistence) 컨텍스트 관리 대상으로 등록한다는 의미(JPA에게 맡김)
        INSERT 발생하는 시점은 현재 메서드가 완료되면서 트랜잭션이 확정될때이다..
        */
        Notice saved=noticeRepository.save(notice);//db에 반영 후 저장된 게시물 반환
        em.refresh(saved); //flush발생 -> INSERT실행 -> SELECT 실행하여 DB값 재로딩
    ...
    //이 메서드는 엔터티를 수정하지 않을 것이라는 의도를 스프링에 명확히 하기 위함 임
    //따라서 JAP가 엔터티에 대한 변경 감지와 같은 불필요한 작업을 하지 않도록, 즉 최적화시키기 위함
    public List<ResponseNotice> getList(){
        //findAll()로 가져온 결과를 우리가 원하는 형태인 List<ResponseNotice>로 변환작업을 해야 함..
        //전통적인 방법으로는 for()문 돌려서 요소를 List 에  add()..  js(배열메서드)  : java(Stream)
    ...
    public ResponseNotice getDetail(Long noticeId){
        ...
        Optional이란? 데이터가 들어있을 수도 있고, 들어있지 않을수도 있음을 객체로 표현한 것
    ...
    public ResponseNotice update(Long noticeId, RequestNotice request){
        ...
        //수정하기 ?  SQL을 직접 건드리는 것이 아니라, 개발자가 Notice 엔터티를 수정하면, JPA가 이 메서드 완료시점에
