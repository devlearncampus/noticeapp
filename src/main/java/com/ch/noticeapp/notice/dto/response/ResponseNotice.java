...
/*JPA 기반으로 개발을 하게 될 경우 테이블과 1:1 매핑되는 Entity클래스가 필요하고,
* 이 클래스는 DTO가 아니다, 하지만 이미 우리가 Notice.java 라는 이름으로 평상시 DTO로 써먹는 이름을
* Entity가 점유하엿으므로, 응답용  DTO를 정의하자 */
@Builder
@Getter
public class ResponseNotice {
    ...
    /*
    * 빌더 패턴을 사용하지 않을 경우, Notice안에있는 데이터들을 생성자를 이용하여 옮길때
    * 코드가 매우복잡해짐 ( 순서를 맞춰야 하니깐..)
